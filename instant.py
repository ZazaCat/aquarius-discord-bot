    ...
    async with aiohttp.ClientSession() as session:
        for query_idx, query in enumerate(queries):
            ...
            for idx, result in enumerate(search_results, start=1):
                ...
                text_content = await process_url(url, session)
                results_json.append({
                    "index": idx,
                    "url": url,
                    "name": name,
...
Use markdown code blocks to write code, including the language for syntax highlighting.
Use LaTeX to wrap ALL math expressions. Always use double dollar signs $$, for example $$E=mc^2$$.
DO NOT include any URLs, only include hyperlinked citations with superscript numbers, e.g. [¹](https://example.com/source1)
DO NOT include references (URLs at the end, sources).
Use hyperlinked footnote citations at the end of applicable sentences (e.g., [¹](https://example.com/source1)[²](https://example.com/source2)).
Write more than 100 words (2 paragraphs).
ALWAYS use the exact cite format provided.
ONLY cite sources from search results below. DO NOT add any other links other than the search results below.
...
Write more than 100 words (2 paragraphs).""", stream=True)
                    ...
                    if user_settings.get(user_id, {}).get("streaming", True):
                        ...
                        await initial_message.edit(content=response.text)
...
def run_streamlit():
    st.title("Discord Bot Admin Panel")
    ...
    def post_to_discord(content):
        ...
        if response.status_code == 204:
            ...
            st.error("Failed to send message. Check Webhook URL.")

    st.header("Announcements")

    announcement = st.text_area("Create an Announcement")
    send_button = st.button("Send Announcement")

    if send_button and announcement:
        post_to_discord(announcement)

    st.header("Other Features")

    if st.button("Mute Bot"):
        post_to_discord("The bot is now muted.")
    if st.button("Unmute Bot"):
        post_to_discord("The bot is now unmuted.")

if __name__ == "__main__":
    # Start the Discord bot in a separate thread
    discord_thread = Thread(target=bot.run, args=(os.getenv('DISCORD_BOT_TOKEN'),))
    discord_thread.start()

    # Start Streamlit as a subprocess
    streamlit_process = subprocess.Popen(['streamlit', 'run', 'streamlit_admin.py'])
