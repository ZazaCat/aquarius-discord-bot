...
api_key = "AIzaSyDp8wSYfJmihGPmW6g18flUojJmEhEmQ5M"  # Replace with your actual API key
...
Google matches your search with keywords in online content. Using too many words may limit results and make it harder to find what you need. Use only the most important keywords when searching. For example:
...
async def process_url(url, session):
    ...
    relevant text using the justext library.
    """
    try:
        async with session.get(url) as response:
            response.raise_for_status()
            html_content = await response.text()
            paragraphs = justext.justext(html_content.encode('utf-8'), justext.get_stoplist("English"))
...
The goal is to provide a text description so overwhelmingly complete that anyone reading it could re-create or vividly imagine the original image in their mind's eye from your words alone. Be as thorough and comprehensive as possible in annotating absolutely everything you perceive in the provided image."""]
                    ...
                    user_text_chat.history.append({
                        "role": "model",
                        "parts": [
                            {"text": "[system](#instructions)\n- Below is the full image description of the image provided by the user, do not share it u til said so. \n[system](#full_image_details)\n" + secondvisionresponse + "[system](#short_image_details)\n" + response.text}]
                    })
...
""")
        ...
        await message.reply("I can't answer that question.", mention_author=False)
        gc.collect()
